...
def nested_tensor_from_tensor_list_easy(tensor_list: List[Tensor]):
    # TODO make this more general
    if tensor_list[0].ndim == 3:
        # TODO make it support different-sized images
        max_size = _max_by_axis([list(img.shape) for img in tensor_list])
        ...
        device = tensor_list[0].device
        tensor = torch.zeros(batch_shape, dtype=dtype, device=device)
        mask = torch.zeros((b, h//32, w//32), dtype=torch.bool, device=device)
        for img, pad_img, m in zip(tensor_list, tensor, mask):
            pad_img[: img.shape[0], : img.shape[1], : img.shape[2]].copy_(img)
            m[: img.shape[1]//32, :img.shape[2]//32] = False
    else:
        raise ValueError('not supported')
    return NestedTensor(tensor, mask)
...
class MetricLogger(object):
    ...
    def log_every(self, iterable, print_freq, header=None):
        ...
        space_fmt = ':' + str(len(str(len(iterable)))) + 'd' # i dont' get it
