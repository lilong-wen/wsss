...
class WSSS(nn.Module):
    ...
    def __init__(self, backbone, args):
        ...
        # fmt: off
        self.text_pos_embed   = PositionalEncoding1D(args.d_model, normalize=True, scale=args.pos_embed_scale)
        # fmt: on
        
        self.num_feature_levels = args.num_feature_levels
        self.num_proposals = args.num_proposals
        self.sigmoid_offset = args.sigmoid_offset
        self.num_refines = args.num_refines

        self.transformer = DeformableTransformer(
            d_model=args.d_model, nhead=args.nhead, num_encoder_layers=args.num_encoder_layers,
            num_decoder_layers=args.num_decoder_layers, dim_feedforward=args.dim_feedforward,
            dropout=args.dropout, activation=args.activation, return_intermediate_dec=args.return_intermediate_dec,
            num_feature_levels=args.num_feature_levels, dec_n_points=args.dec_n_points, 
            enc_n_points=args.enc_n_points, num_proposals=args.num_proposals,
        )
        self.ctrl_point_class = nn.Linear(args.d_model, args.num_classes)
        # self.ctrl_point_coord = MLP(args.d_model, args.d_model, 2, 3)
        #TODO change this to xyxyxy.. format instead of xyhw
        self.ctrl_point_coord = MLP(args.d_model, args.d_model, 1, 3)
        self.bbox_coord = MLP(args.d_model, args.d_model, 4, 3)
        self.bbox_class = nn.Linear(args.d_model, args.num_classes)
        self.text_class = nn.Linear(args.d_model, args.voc_size + 1)

        # shared prior between instances (objects)
        self.ctrl_point_embed = nn.Embedding(args.num_ctrl_points, args.d_model)
        self.text_embed = nn.Embedding(args.max_text_len, args.d_model)
        ...
        if args.num_feature_levels > 1:
            ...
            for _ in range(num_backbone_outs):
                ...
                input_proj_list.append(nn.Sequential(
                    nn.Conv2d(in_channels, args.d_model, kernel_size=1),
                    nn.GroupNorm(32, args.d_model),
                ))
            for _ in range(args.num_feature_levels - num_backbone_outs):
                input_proj_list.append(nn.Sequential(
                    nn.Conv2d(in_channels, args.d_model,
            ...
            self.input_proj = nn.ModuleList([
                nn.Sequential(
                    nn.Conv2d(
                        num_channels[0], args.d_model, kernel_size=1),
                    nn.GroupNorm(32, args.d_model),
                )])
        self.aux_loss = True
        ...
        self.ctrl_point_class.bias.data = torch.ones(args.num_classes) * bias_value
        self.bbox_class.bias.data = torch.ones(args.num_classes) * bias_value
        ...
        num_pred = args.num_decoder_layers
    ...
    def forward(self, samples: NestedTensor, texts):
        ...
        features_out, pos = self.backbone(samples, texts)
        features = features_out['img_feature']
        ...
        for lvl in range(hs.shape[0]):
            ...
            if reference.shape[-1] == 2:
                ...
                tmp += reference[:, :, None, :1]
                #TODO change to xyxyxy.. format instead of xyhw
